...
Reference/              # Rules, cultures, callings, weapons, adversaries
...
**Reference files** in `Reference/` follow a consistent callout structure using `[!combat]`, `[!Council]`, `[!Rolls]` blocks. Match this style when adding new reference material.
...
- New adversary stat blocks in `Reference/Adversaries/`
...
- **New reference material:** Match the style of existing files in `Reference/`. Use callout blocks for rules excerpts. Don't reproduce copyrighted text — summarize or reference PDF page numbers.
...
- **New adversaries:** Follow the YAML stat block format in `Reference/Adversaries/` — the TOR 2E Statblocks plugin renders these.
- **Rules questions:** Reference `Reference/Rules.md` and `Reference/TOR Cheat Sheet.md` before asking to look elsewhere.
